...
import { LayoutGrid, Upload, FileText, Loader2 } from "lucide-react";
import { cn } from "../lib/utils";

export default function CreateDeckPicklist() {
    ...
    async function processFiles(files: File[]) {
        if (files.length > 0) {
            ...
            setIsLoading(true);
            try {
                await CallApiWithFile(files, apiPaths.CreateDeckPicklist, 'DeckPicklist.txt');
            } finally {
                setIsLoading(false);
            }
        }
    ...
    return (
        <div className="space-y-8 p-4">
            <div className="text-center space-y-4">
                ...
                >
                    <LayoutGrid className="text-purple-400" size={40} />
                </motion.div>
                <div className="space-y-2">
                    <h2 className="text-4xl font-black tracking-tight text-white uppercase tracking-tight">Create Picklist</h2>
                    <p className="text-slate-400 text-lg max-w-xl mx-auto italic">
                        "Your deck, organized for gathering."
                    </p>
                </div>
            </div>
                ...
                >
                    ...
                    <div className="text-center">
                        <p className="text-lg font-bold text-slate-100">
                            {isLoading ? "Generating Picklist..." : "Drop deck files for picklist"}
                        </p>
                        <p className="text-slate-400 text-sm">
                            suitable for gathering physical cards
                        </p>
                    </div>
